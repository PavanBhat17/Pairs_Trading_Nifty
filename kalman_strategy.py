...
def handle_missing_data_kalman(df):
    """
    Impute missing values in the IV data using block-wise Kalman filter. Falls back to linear interpolation for large or problematic gaps, and handles edge NaNs with forward/backward fill. Adds an 'is_interpolated' column to mark imputed rows.
    ...
    for col in ['nifty', 'banknifty']:
        ...
        if series.notna().sum() == 0:
            print(f"[SKIP] All values in '{col}' are NaN or inf.")
            continue
        # Fill initial/final NaNs with ffill/bfill as fallback
        series = series.fillna(method='ffill').fillna(method='bfill')
        # Block-wise Kalman imputation
        ...
        for i, is_nan in enumerate(nan_mask):
            if is_nan and not in_block:
                ...
                in_block = False
        if in_block:
            nan_blocks.append((block_start, len(series)-1))
        for start, end in nan_blocks:
            block_len = end - start + 1
            win_start = max(0, start-250)
            win_end = min(len(series), end+251)
            window = series.iloc[win_start:win_end].copy()
            if window.notna().sum() < 50:
                # fallback to linear interpolation for this block
                series.iloc[start:end+1] = np.interp(
                    np.arange(start, end+1),
                    np.flatnonzero(~window.isna()),
                    window[~window.isna()]
                )
                warnings.warn(f"[{col.upper()}] Block {start}-{end} imputed with linear interpolation (too few points for Kalman)")
                continue
            # Pre-fill window
            ...
            kf = KalmanFilter(
                transition_matrices=[1],
                observation_matrices=[1],
                initial_state_mean=window.iloc[0],
                observation_covariance=1,
                transition_covariance=0.01
            )
            try:
                kf = kf.em(window.values, n_iter=3)
                state_means, _ = kf.smooth(window.values)
                smoothed = pd.Series(state_means.flatten(), index=window.index)
                series.iloc[start:end+1] = smoothed.iloc[(start-win_start):(end-win_start+1)].values
                if block_len > 20:
                    warnings.warn(f"[{col.upper()}] Large block {start}-{end} imputed with Kalman (length {block_len})")
            except Exception:
                ...
                warnings.warn(f"[{col.upper()}] Block {start}-{end} imputed with linear interpolation (Kalman failed)")
        df_filled[col] = series
    df_filled['is_interpolated'] = df[['nifty', 'banknifty']].isna().any(axis=1).astype(np.int8)
    return df_filled
...
def main():
    ...
    try:
        ...
        df = handle_missing_data_kalman(df)
